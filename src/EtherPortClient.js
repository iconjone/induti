...
function EtherPortClient(opts) {
  ...
  this._reconnectTimeoutSecs = opts.reconnectTimeout || 1; //15
  this._queue = [];
  this._socket = null;
  ...
  this._tcp.on("connect", () => {
    this._tcp.status = true;
    opts.callback("electronStoreEtherport_connect");
  });
  this._tcp.on("data", this._onDataHandler());
  this._tcp.on("error", this._onErrorHandler());
  this._tcp.on("error", () => {
    this._tcp.status = false;
    opts.callback("electronStoreEtherport_error");
  });
  this._tcp.on("timeout", this._onTimeoutHandler());
  this._tcp.on("timeout", () => {
    this._tcp.status = false;
    opts.callback("electronStoreEtherport_timeout");
  });
  this._tcp.on("close", this._onCloseHandler());
  // this._tcp.on("close", () => {
  //   this._tcp.status = false;
  //   opts.callback("electronStoreEtherport_close");
  // });
  this._connect();
}
...
EtherPortClient.prototype._reconnect = function () {
  ...
  if (self._reconnectTimeoutSecs > 0) {
    debug(
      util.format("Reconnect after %d second(s)", self._reconnectTimeoutSecs)
    );
...
EtherPortClient.prototype.destroy = function () {
